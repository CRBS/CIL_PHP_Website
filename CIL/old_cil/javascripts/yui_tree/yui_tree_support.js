 ...
 *                   potentially selectable, unless they too are excluded. IDs
 *                   are a caller-specific concept and should be expressed as
 *                   strings or integers.
 *   --------------------------------------------------------------------------
...
function uk_org_pond_yui_tree_support( options )
{
  this.onPageLoad = function()
  {
    var parentHltList = [];
    var tree;
    var formField, nameField, selectedLabelFieldId;
    ...
    if( options.selectedLabelFieldId ) selectedLabelFieldId = document.getElementById( options.selectedLabelFieldId);
    ...
    {
      var sUrl     = options.xhrURL + "?tree_parent_id=" + encodeURI( node.data.org_uk_pond_yui_tree_id );
      var callback =
      {
    ...
    function nodeClickedUpon( event )
    {
      ...
      /*
       * Write out the text value to the hidden field
       * */
       selectedLabelFieldId.value = node.label;
      return false;
    }
